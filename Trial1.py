...
def get_price():
    ...
    for topic in stock_list:
        ...
        if topic in ("N225", "TOPX", "JSD", "MTHR", "TSI2"):
            price = {
                "symbol": dde.request("æŒ?æ¨™ã‚³ãƒ¼ãƒ?").decode('sjis'),
                "symbol_name": dde.request("æŒ?æ¨™åç§°").decode('sjis'),
                "get_date": dde.request("ç¾åœ¨æ—¥ä»?").decode('sjis'),
                "open_price": dde.request("å§‹å?¤").decode('sjis'),
                "high_price": dde.request("é«˜å?¤").decode('sjis'),
                "low_price": dde.request("å®‰å?¤").decode('sjis'),
                "close_price": dde.request("ç¾åœ¨å€¤").decode('sjis'),
                "volume": "",
                "trading_value": "",
            }
        else:
            price = {
                "symbol": dde.request("éŠ˜æŸ„ã‚³ãƒ¼ãƒ?").decode('sjis').strip(),
                "symbol_name": dde.request("éŠ˜æŸ„åç§°").decode('sjis').strip(),
                "get_date": dde.request("ç¾åœ¨æ—¥ä»?").decode('sjis').strip(),
                "open_price": dde.request("å§‹å?¤").decode('sjis').strip(),
                "high_price": dde.request("é«˜å?¤").decode('sjis').strip(),
                "low_price": dde.request("å®‰å?¤").decode('sjis').strip(),
                "close_price": dde.request("ç¾åœ¨å€¤").decode('sjis').strip(),
                "volume": dde.request("å‡ºæ¥é«?").decode('sjis').strip(),
                "trading_value": dde.request("å£²è²·ä»£é‡?").decode('sjis').strip(),
            }
        print(price)
